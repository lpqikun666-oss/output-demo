...
## 具体原理
  1. 合成影像本质上是一张画出来的图 — generate_synthetic_rs_image() 用 PIL画了湖泊、森林、农田方块、灰色楼房、棕色裸地，加模糊和噪声让它看起来像卫星图
  2. K-Means 聚类只看颜色 — 它把每个像素的 RGB 值扔进聚类算法，颜色相近的自动分到一组。因为画图时用了不同的颜色画不同地物，聚类结果恰好对应了"蓝色=水体、深绿=森林、灰=城镇"这种物理意义
  3. 这是一个"技术演示"，不是"真实分析" — 它展示了 K-Means在遥感影像分类中的应用流程（降采样→聚类→统计占比→可视化），但输入是模拟的而不是真实的 Landsat / Sentinel 影像
...
请再次注意：该流程并非分析真实的遥感数据，而是在演示技术流程。所以无需任何外部数据 — 程序会自动生成合成遥感影像并进行分析。所有输出保存在 `results/` 文件夹。
